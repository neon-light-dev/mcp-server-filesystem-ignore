# `@neonlight/mcp-server-filesystem-ignore`

`@neonlight/mcp-server-filesystem-ignore` is a standalone repository and npm package derived from the official Model Context Protocol filesystem server in [`modelcontextprotocol/servers`](https://github.com/modelcontextprotocol/servers/tree/main/src/filesystem).
...
```bash
npm install -g @neonlight/mcp-server-filesystem-ignore
```

You can also run it without a global install:

```bash
npx -y @neonlight/mcp-server-filesystem-ignore /absolute/project/path
```
...
```bash
npx -y @neonlight/mcp-server-filesystem-ignore \
...
4. Run `npm login`.
5. Run `npm run build`.
6. Run `npm pack --dry-run` and confirm the tarball only contains `dist` plus the intended docs and license files.
7. Publish with `npm publish --access public`.
8. Verify the published version with `npm view @neonlight/mcp-server-filesystem-ignore version`.
9. Verify the package page on npm shows the expected README, license, and install command.
10. Use normal semantic versioning for this fork independently of upstream.
...
- Trusted publishing through GitHub Actions is a good follow-up, but it is optional for this initial fork.
